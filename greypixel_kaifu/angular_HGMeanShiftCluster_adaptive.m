function [clustCent,data2cluster,cluster2dataCell] = angular_HGMeanShiftCluster(dataPts,knndivider,kernel,plotFlag);
...
%     old_dataPts=dataPts;
    while true %loop untill convergence
        ...
        sqDistToAll=sqDistToAll';
        [sortedDist,indexDist] = sort(sqDistToAll);
        bandWidth = sortedDist(knn);
        inInds = find(sqDistToAll < bandWidth); % points within bandWidth
        ...
        if acos(dot(myMean',myOldMean',2))< stopThresh  %******************
        %if norm(myMean-myOldMean) < stopThresh
            %check for merge posibilities
            mergeWith = 0;
            for cN = 1:numClust
                %distToOther = norm(myMean-clustCent(:,cN)); % distance to old clust max
                distToOther = acos(dot(myMean',clustCent(:,cN)',2)); %******************
