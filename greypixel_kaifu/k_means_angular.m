function [gIdx,c]=k_means_angular(X,k)
...
if ~isscalar(k)  % input center points
    c=k;
    k=size(c,1);
else
    rng(1234);
    c=X(ceil(rand(k,1)*n),:);  % randomly generate the center points
end
...
%         end
        center=c(t,:);
        X_normed=normr(X); %(n,1)
        greylight=normr(repmat(center,[n,1])); %(n,1)
        angular_error=real(acos(dot(X_normed,greylight,2))); %shape(n,1) value0~0.955
        d=angular_error;%(n,1)
        D(:,t)=d; %(n,k)
    end
