function [clustCent,data2cluster,cluster2dataCell] = HGMeanShiftCluster_weighted(dataPts,bandWidth,weight,kernel,plotFlag);
...
stopThresh = 1e-3*norm(bandWidth); % when mean has converged
...
case 'gaussian' % approximated gaussian kernel
    kmean = @(x,d) gaussfun(x,d,norm(bandWidth));
case 'weighted_mean' % 
    kmean = @(x,dis,weight) mean(x.*weight,2);
case 'weighted_gaussian' % approximated gaussian kernel
    kmean = @(x,d,weight) gaussfun_weighted(x,d,norm(bandWidth),weight);
otherwise
...
while numInitPts
    ...
    while true %loop untill convergence
        ...
        myMean = kmean(dataPts(:,inInds),sqrt(sqDistToAll(inInds)),weight(:,inInds)); % compute the new mean
...
%         end
        ...
        if norm(myMean-myOldMean) < stopThresh
            ...
            for cN = 1:numClust
                ...
                if distToOther < norm(bandWidth)/2 % if its within bandwidth/2 merge new and old
                    mergeWith = cN;
                    break;
                end
            end
