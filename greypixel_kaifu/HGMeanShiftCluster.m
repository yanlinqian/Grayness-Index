function [clustCent,data2cluster,cluster2dataCell] = HGMeanShiftCluster(dataPts,bandWidth,kernel,greyness,plotFlag);
...
stopThresh = 1e-2*norm(bandWidth); % when mean has converged
...
case 'epanechnikov' %epanechnikov kernel
    kmean = @(x,d) epanechnikov(x,d,norm(bandWidth));
otherwise
